(function ($) {
    ...
    function onPlaceAddClick(e) {
        ...
        $(".leaflet-popup-content-wrapper").on('click', function (e) {
            ...
            var url = "http://demo.places.nlp.nokia.com/places/v1/places/PLACEID?app_id=EOXbMEWYAllPhQnAQsmn&app_code=9TIppnJDB9PHy8-ckJLWXA";

            // Replacing parameters in the request
            var urlrequest = url.replace("PLACEID", placeId);
    ...
    function onMarkerClick(e) {
        ...
        $.getJSON('SaltedButter/Index?placeId=' + id, function (data) {
            openNote(data[0], false)
        });

    }

    function openNote(data, add) {
        var place;
        $("#modal-info").removeClass("hide");

        if (add) {
            place = mapNokiaPlaceToPlace(data);
            $(".control-group").removeClass("hide");
            $(".defined").addClass("hide");
        }
        else {
            place = mapQPlaceToPlace(data);
            $(".control-group").addClass("hide");
            $(".defined").removeClass("hide");
            $("#place-name").text(place.Name);
            $("#place-address").html(place.Vicinity);
            setNoteStatus(place.Type);
            setNoteSaltedButter(data);
            setNoteAuthor(place.Username, place.CreationDate);
        }
    }

    function setNoteStatus(salted) {
        var imageSource = place.Type === "salted" ? 'static/img/pinvert.png' : 'static/img/pinrouge.png';
        var imageText = place.Type === "salted" ? "Adresse sal&eacute;e" : "Adresse non sal&eacute;e";
        $("#status-image").attr("src", imageSource);
        $("#status-text").html(imageText);
    }

    function setNoteSaltedButter(qPlace) {
        $("#plate-text").html(qPlace.SaltedButter.Plate);
        $("#comment-text").html(qPlace.SaltedButter.Comment);
    }

    function setNoteAuthor(username, creationdate) {

        $("#author-name").html(username);

        var currentDate = new Date();
        //Have to get the number of ticks as JSON serialization passes a non formatted date
        var patt = /\d+/g;
        var result = patt.exec(creationdate)[0];
        var createDate = new Date(parseInt(result, 10));
        var hour = Math.round((currentDate.getTime() - createDate.getTime()) / 1000 / 3600);
        var day = Math.round((currentDate.getTime() - createDate.getTime()) / 1000 / 3600 / 24);
        var month = Math.round((currentDate.getTime() - createDate.getTime()) / 1000 / 3600 / 24 / 30);
        var year = Math.round((currentDate.getTime() - createDate.getTime()) / 1000 / 3600 / 24 / 365);

        if (hour < 1) {
            $("#add-time").text("moins d'une heure");
        }
        else if (hour == 1) {
            $("#add-time").text("une heure");
        }
        else if (hour < 24) {
            $("#add-time").text(hour + " heures");
        }
        else if (day == 1) {
            $("#add-time").text("un jour");
        }
        else if (day < 31) {
            $("#add-time").text(day + " jours");
        }
        else if (month < 13) {
            $("#add-time").text(month + " mois");
        }
        else if (year == 1) {
            $("#add-time").text("un an");
        }
        else if (year > 1) {
            $("#add-time").text(year + " ans");
        }
    }

    function setComment(qPlace) {
        $("#comment-text").text(qPlace.SaltedButter.Comment);
    }
